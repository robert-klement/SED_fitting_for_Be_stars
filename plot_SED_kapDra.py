...
for j, star in enumerate(starlist):
    ...
    ###
    sp = S.Icat('ck04models', 13982, 0.0, 3.479) # kap Dra
    sp = sp * S.Extinction(0.022, 'mwavg')
    sp.convert('fnu')

    factor = (((5.85*u.Rsun) / (142.7*u.pc))**2).decompose()
    flux_nu_Be = sp.flux * factor

    ax1.plot(sp.wave*1e-4, flux_nu_Be, color='grey', lw=0.5)

    # ###
    # sp = S.Icat('ck04models', 16700, 0.0, 4.75) # kap Dra
    # sp = sp * S.Extinction(0.022, 'mwavg')
    # sp.convert('fnu')

    # factor = (((0.690*u.Rsun) / (142.7*u.pc))**2).decompose()
    ...
    # ax1.axhline(y=1.346e-23, xmin=0.13-0.08, xmax=0.17-0.08, color='red',ls='--', lw=0.8)
    ax1.axhline(y=1.639e-23, xmin=0.13-0.08, xmax=0.17-0.075, color='k',ls='--', lw=0.6)
    ax1.axhline(y=1.053e-23, xmin=0.13-0.08, xmax=0.17-0.075, color='k',ls='--', lw=0.6)
