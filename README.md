...
- `make_phot_file_*.py` + `merge_phot_files.py`: scripts to create cleaned photometry files by  and merge them into one.
  - AKARI, IRAS, and WISE - downloads data from the corresponding Vizier catalogs, discards flagged data points, performs color correction and unit coversion into Jy when needed
  - Simbad - downloads magnitudes from Simbad
  - Dougherty - reads Johnson magnitudes *_dougherty_johnson.dat, which needs to be created manually based on Table 2 of [Dougherty et al. (1991)](https://ui.adsabs.harvard.edu/abs/1991AJ....102.1753D/) near-IR photometry paper.
