...
- `plot_SED.py`: creates the SED plot from merged photometry, and IUE and HPOL averaged spectra. Also fits a a power law to the IR photometry in a selected interval, and converts it to the density slope in Be star disks following Eq.20 of Vieira et al. (2015). It also included optional dereddening if you input a non-zero value of E(B-V) in the script.
- `omeOri/`: example outputs from these SED scripts along with manually downloaded input spectra from IUE and HPOL, and manually created photometry file photometry_Doughery.dat taken from Dougherty et al. (1991) near-IR photometry paper.
